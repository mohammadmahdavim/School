...
//        گرفتن آیتم های مربوط به این درس
            ...
            return view('Admin.student_mark.mark', compact('users', 'iddars', 'darss', 'items', 'idc'));
        } else {
    ...
    function edit(Request $request)
    {
        if (auth()->user()->role == 'معلم' or auth()->user()->role == 'مدیر') {
            ...
            $teacher = teacher::where('dars', $iddars)->first();
    ...
    function create($idc, $idd)
    {
        if (auth()->user()->role == 'معلم' or auth()->user()->role == 'مدیر') {
            ...
            $classid = $idc;
            $items = CMark::where('dars', $darss->id)->where('classid', $classid)->count();

            return view('Admin.student_mark.createmarkss', compact('idd', 'darss', 'items', 'classid', 'idc'));
        } else {
            return view('errors.404');
        }
    }
...
//                    'name' => 'required',
            ...
            return redirect(url('admin/mark/' . $idc . '/' . $idd));
        } else {
            return view('errors.404');
        }
    }
    ...
    function viewmark($idc, $idd)
    {
        if (auth()->user()->role == 'معلم' or auth()->user()->role == 'مدیر') {
            ...
            return view('Admin.student_mark.viewmark', compact('cmark', 'idd', 'dars', 'idc'));
        } else {
            return view('errors.404');
        }
    }
    ...
    function editeemark(Request $request, $id)
    {
        ...
        if (auth()->user()->role == 'معلم' or auth()->user()->role == 'مدیر') {
            ...
            $mark = CMark::where('id', $id)->first();
            $teacher = teacher::where('class_id', $mark->classid)->first();
            if ($teacher->class_id = $mark->classid) {
                $mark->update([
                    'name' => request('name'),
                    'updated_at' => Jalalian::now(),
                ]);;
    ...
    public function date($class, $dars)
    {
        ...
        return view('Admin.student_mark.markdate', compact('class', 'dars'));
    }
